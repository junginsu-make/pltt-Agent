...
Task tool parameters:
- subagent_type: "backend-specialist"
- description: "Phase 1, T1.1: 휴가 API 구현"
- prompt: |
    ## 태스크 정보
    - Phase: 1
    - 태스크 ID: T1.1
    ...
    ```bash
    git worktree add ../palette-phase1-leave -b phase/1-leave
    cd ../palette-phase1-leave
    ```

    ## TDD 요구사항 (Phase 1+ 필수!)
    반드시 TDD 사이클을 따르세요:
    1. RED: 테스트 먼저 작성 (services/leave-service/tests/)
    2. GREEN: 테스트 통과하는 최소 구현
    3. REFACTOR: 테스트 유지하며 코드 정리

    테스트 명령어: `pnpm --filter leave-service test`
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../palette-phase1-leave`
```
...
| subagent_type | 역할 |
|---------------|------|
| `backend-specialist` | Hono API, 비즈니스 로직, 서비스 간 통신 |
| `frontend-specialist` | Next.js 14 UI, shadcn/ui, Zustand, Socket.IO |
| `database-specialist` | Drizzle ORM, PostgreSQL 마이그레이션 |
